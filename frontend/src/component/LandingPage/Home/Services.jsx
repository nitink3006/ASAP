      ...
      try {
        ...
        const filteredServices =
          storedZip && storedZip !== "null"
            ? data.filter((service) => service.zip_code === storedZip || service.zip_code === "0")
            : data;

        setServices(filteredServices);      } catch (error) {
    ...
    <div className="bg-white p-6 rounded-lg shadow-lg w-96 max-w-full border-2 border-gray-300 relative">
      ...
      <div className="grid grid-cols-3 gap-4 mt-3">
        {subCategories.length > 0 ? (
          ...
          <p className="text-gray-500 text-center mt-2">No subcategories found</p>
        )}
      </div>
    </div>
  </div>
